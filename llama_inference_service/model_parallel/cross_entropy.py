...
class _VocabParallelCrossEntropy(torch.autograd.Function):
    ...
    def forward(ctx, vocab_parallel_logits, target):  # type: ignore
        ...
        torch_gcu.distributed.all_reduce(logits_max, op=torch_gcu.distributed.ReduceOp.MAX, group=get_model_parallel_group())
        ...
        torch_gcu.distributed.all_reduce(
            predicted_logits, op=torch_gcu.distributed.ReduceOp.SUM, group=get_model_parallel_group()
        )
        ...
        torch_gcu.distributed.all_reduce(
            sum_exp_logits, op=torch_gcu.distributed.ReduceOp.SUM, group=get_model_parallel_group()
        )
