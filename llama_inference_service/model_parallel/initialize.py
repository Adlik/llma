...
import torch
import sys
# sys.path.append("/home/xin.qu/llma/llama/model_parallel")
# sys.path.append("/home/xin.qu/llma/llama/llama")
from .utils import ensure_divisibility
# sys.path.append("/home/xin.qu/llma/llama")
from model_parallel import is_torch_gcu_available
...
def initialize_model_parallel(
    ...
    *,
    ...
    assert torch_gcu.distributed.is_initialized()
    world_size = torch_gcu.distributed.get_world_size()
    ...
    rank = torch_gcu.distributed.get_rank()

    data_parallel_size = int(world_size / (model_parallel_size * pipeline_length))

    if torch_gcu.distributed.get_rank() == 0:
    ...
    for j in range(pipeline_length):
        for k in range(model_parallel_size):
            group = torch_gcu.distributed.new_group(groups[:, j, k].tolist(), backend=ddp_backend)
    ...
    for i in range(data_parallel_size):
        for j in range(pipeline_length):
            group = torch_gcu.distributed.new_group(groups[i, j, :].tolist(), backend=model_parallel_backend)
    ...
    for i in range(data_parallel_size):
        for k in range(model_parallel_size):
            ...
            group = torch_gcu.distributed.new_group(ranks, backend=pipeline_backend)
...
def get_model_parallel_group() -> torch_gcu.distributed.ProcessGroup:
    """Get the model parallel group the caller rank belongs to."""
    assert _MODEL_PARALLEL_GROUP is not None, "model parallel group is not initialized"
    return _MODEL_PARALLEL_GROUP


def get_data_parallel_group() -> torch_gcu.distributed.ProcessGroup:
    """Get the data parallel group the caller rank belongs to."""
    assert _DATA_PARALLEL_GROUP is not None, "data parallel group is not initialized"
    return _DATA_PARALLEL_GROUP


def get_pipeline_parallel_group() -> torch_gcu.distributed.ProcessGroup:
...
def get_model_parallel_world_size() -> int:
    ...
    return torch_gcu.distributed.get_world_size(group=get_model_parallel_group())


def get_model_parallel_rank() -> int:
    """Return my rank for the model parallel group."""
    return torch_gcu.distributed.get_rank(group=get_model_parallel_group())


def get_model_parallel_src_rank() -> int:
    """Calculate the global rank corresponding to a local rank zero
    in the model parallel group."""
    global_rank = torch_gcu.distributed.get_rank()
...
def get_data_parallel_world_size() -> int:
    ...
    return torch_gcu.distributed.get_world_size(group=get_data_parallel_group())


def get_data_parallel_rank() -> int:
    """Return my rank for the data parallel group."""
    return torch_gcu.distributed.get_rank(group=get_data_parallel_group())
