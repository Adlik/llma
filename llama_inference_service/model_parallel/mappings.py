...
def _reduce(ctx: Any, input_: torch.Tensor) -> torch.Tensor:
    ...
    if torch_gcu.distributed.get_world_size(group=group) == 1:
        return input_
    # All-reduce.
    torch_gcu.distributed.all_reduce(input_, group=group)
...
def _split(input_: torch.Tensor) -> torch.Tensor:
    ...
    world_size = torch_gcu.distributed.get_world_size(group=group)
    input_list = split_tensor_along_last_dim(input_, world_size)

    # Note: torch.split does not create contiguous tensors by default.
    rank = torch_gcu.distributed.get_rank(group=group)
...
def _gather(input_: torch.Tensor) -> torch.Tensor:
    ...
    torch_gcu.distributed.all_gather(tensor_list, input_, group=group)
